...
class SignApkJob(
    ...
    private val keySpec: ApkSigningKeySpec
) : FileJob() {
    @Throws(IOException::class)
    override fun run() {
        val fileName = getFileName(file)
        postSignApkNotification(fileName)
        val key = try {
            ApkSigning.resolveKey(service, keySpec)
        } catch (e: Exception) {
            e.printStackTrace()
            throw IOException(getString(R.string.file_sign_apk_error_key_format, e.toString()), e)
        }
        val signApkCacheDirectory = File(cacheDirectory, "sign_apk_cache").apply { mkdirs() }
        val inputCacheFile = File(signApkCacheDirectory, "input_$id.apk")
        val outputCacheFile = File(signApkCacheDirectory, "output_$id.apk")
        try {
            outputCacheFile.delete()
            // Copy the source into local cache so apksig can random-access it regardless of the
            // underlying file system (archive, SFTP, SMB, ...).
            file.newInputStream().use { input ->
                inputCacheFile.outputStream().use { output -> input.copyTo(output) }
            }
            throwIfInterrupted()
            try {
                ApkSigning.sign(inputCacheFile, outputCacheFile, key)
            } catch (e: Exception) {
                e.printStackTrace()
                throw IOException(
                    getString(R.string.file_sign_apk_error_sign_format, fileName, e.toString()), e
                )
            }
            throwIfInterrupted()
            val target = resolveSignedApkTarget(fileName)
            outputCacheFile.inputStream().use { input ->
                target.newOutputStream(
                    StandardOpenOption.CREATE_NEW, StandardOpenOption.WRITE
                ).use { output -> input.copyTo(output) }
            }
            showToast(getString(R.string.file_sign_apk_success_format, getFileName(target)))
        } finally {
    ...
    private fun resolveSignedApkTarget(fileName: String): Path {
        val baseName = fileName.substringBeforeLast('.', fileName)
        var candidate = file.resolveSibling("${baseName}_signed.apk")
        var index = 2
        while (candidate.exists()) {
            candidate = file.resolveSibling("${baseName}_signed_$index.apk")
            index++
        }
        return candidate
    }
...
private fun FileJob.postSignApkNotification(fileName: String) {
    postNotification(
        getString(R.string.file_job_sign_apk_notification_title_format, fileName), null, null, null,
        0, 0, true, false
    )
}
